...
st.sidebar.markdown(
    "<h1 style='text-align: center;'>AIONA Front-end</h1>",
...
if 'screen_design_tab' not in st.session_state:
    st.session_state.screen_design_tab = 'main'
...
def render_design_product_component_page(key: str = "Design"):    
    st.title(f"Search {key}")
    
    search_term = st.text_input('Search:', '', key=key)
    xlsx_path = 'assets/sample.xlsx'
    ...
    # Header
    header_cols = st.columns([5] + [5]+ [20] * (len(display_data.columns) - 1) + [15])
    with header_cols[0]:
        st.markdown(f"<div style='text-align: center; font-weight: bold;'>INDEX</div>", unsafe_allow_html=True)
        
    for i, column in enumerate(display_data.columns):
        with header_cols[i + 1]:
            st.markdown(f"<div style='text-align: center; font-weight: bold;'>{column.upper()}</div>", unsafe_allow_html=True)
            
    with header_cols[len(display_data.columns) + 1]:
        st.markdown(f"<div style='text-align: center; font-weight: bold;'>PREVIEW</div>", unsafe_allow_html=True)
    st.markdown("---")
    
    # Display table data with preview button
    for idx, row in display_data.iterrows():
        cols = st.columns([5] + [5] + [20] * (len(row) - 1) + [15])
        with cols[0]:
            st.markdown(f"<div style='text-align: center;'>{idx + 1}</div>", unsafe_allow_html=True)
        for i, column in enumerate(row.index):
            with cols[i + 1]:
                st.markdown(f"<div style='text-align: center;'>{row[column]}</div>", unsafe_allow_html=True)
        with cols[len(row) + 1]:
            if st.button("Preview PDF", key=f"preview_{idx}"):
                st.session_state.screen_design_tab = f"{row['ID']}"
                st.rerun()
        st.markdown("---")
...
def render_screen_component_page(key: str = "Component"):
    render_search_table(key=key)
...
def render_search_table(key: str):
    st.title(f'Search {key}')
...
if authentication_status == False:
    ...
    elif st.session_state.screen == 'incident':
        render_search_table(key="Incident")
                
    elif st.session_state.screen == 'design':
        
        if st.session_state.screen_design_tab == 'main':
            render_design_product_component_page()
        else:
            # Extract the item ID from the active tab name
            item_id = int(st.session_state.screen_design_tab)
            render_screen_component_page(key="Component")